...
var importCmd = &c.Command{
	...
	Run: func(cmd *c.Command, args []string) {
		...
		files := h.ReadDir("/app/temp/helm-secret")
		...
		for _, file := range files {
			...
			name := h.ReplaceAllString(file, "/app/temp/helm-secret/sh\\.helm\\.release\\.v1\\.([\\w-]+)\\.v\\d+\\.json", "$1")
			version := h.ReplaceAllString(file, "/app/temp/helm-secret/sh\\.helm\\.release\\.v1\\.[\\w-]+\\.v(\\d+)\\.json", "$1")
